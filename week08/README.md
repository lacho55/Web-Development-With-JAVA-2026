...
We want the DB-backed code to be opt-in, so we will introduce a new Spring profile `db`
and leave the existing `dev` profile (with its in-memory repositories) untouched for now.

Create `project/src/main/resources/application-db.yml`:
...
To run the app on this profile, either set the active profile in `application.yml`
(`spring.profiles.active: db`) or pass `--spring.profiles.active=db` when starting.
...
Start the app with the `db` profile. You should see Liquibase logs followed by a
...
`DevInMemoryShowRepository` (a `HashMap<Long, Show>` with `@Profile("dev")` and a
`@PostConstruct` seed). We will add a DB-backed implementation alongside it, active only
under the `db` profile.
...
Because `DevInMemoryShowRepository` is `@Profile("dev")` and `DbShowRepository` is
`@Profile("db")`, Spring will wire exactly one `ShowRepository` bean based on the active
profile — no other service code has to change.

### 4.3 Optional — seed some data

Either add an `INSERT` changeset after `001-create-show-table.yaml`, or copy the seed
shows from `DevInMemoryShowRepository.seed()` into a small `CommandLineRunner` that only
runs under `@Profile("db")` when the table is empty.

---

# Task 5 — Test through the REST API

Run the app with `--spring.profiles.active=db` and exercise `ShowController` via Postman
