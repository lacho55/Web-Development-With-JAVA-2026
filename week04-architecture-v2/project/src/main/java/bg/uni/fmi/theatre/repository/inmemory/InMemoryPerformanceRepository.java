...
public class InMemoryPerformanceRepository implements PerformanceRepository {
    ...
    @Override
    public Performance save(Performance p) { store.put(p.getId(), p); return p; }

    @Override
    public Optional<Performance> findById(Long id) { return Optional.ofNullable(store.get(id)); }

    @Override
    public List<Performance> findAll() { return List.copyOf(store.values()); }
